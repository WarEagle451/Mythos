...
namespace myl::vulkane {
	...
	fence::~fence() {
		if (m_handle)
			vkDestroyFence(m_context.device().logical(), m_handle, nullptr);
	}

	bool fence::wait(u64 a_max) {
		if (m_signaled)
			return true;
		else switch (vkWaitForFences(m_context.device().logical(), 1, &m_handle, true, a_max)) {
			case VK_SUCCESS:
				m_signaled = true;
				return true;
			case VK_TIMEOUT:
			...
			case VK_ERROR_OUT_OF_HOST_MEMORY:
				MYL_CORE_ERROR("Vulkan fence - VK_ERROR_OUT_OF_HOST_MEMORY");
				break;
	...
	void fence::reset() {
		if (m_signaled) {
			MYL_VK_CHECK(vkResetFences, m_context.device().logical(), 1, &m_handle);
			m_signaled = false;
		}
	}
}