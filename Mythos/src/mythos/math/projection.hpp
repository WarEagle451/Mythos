...
namespace myl {
	...
	template<typename T>
	MYL_NO_DISCARD constexpr mat4x4<T> perspective(T fov_radians, T aspect, T a_near, T a_far) {
	...
	template<typename T>
	MYL_NO_DISCARD constexpr mat4x4<T> look_at(const vec3<T>& target, const vec3<T>& eye, const vec3<T>& up) {
