...
namespace myl {
	...
	protected:
		...
		MYL_API constexpr key_event(const key_code a_code)
			: m_code(a_code) {}
	public:
		MYL_API constexpr key_code key() const { return m_code; }
	...
	class key_pressed_event : public key_event {
		u16 m_repeat_count;
	public:
		MYL_API constexpr key_pressed_event(const key_code a_code, const u16 a_repeat_count)
			: key_event(a_code)
			, m_repeat_count(a_repeat_count) {}

		MYL_API constexpr u16 repeat_count() const { return m_repeat_count; }
	...
	public:
		MYL_API key_released_event(const key_code a_code)
			: key_event(a_code) {}
	...
	public:
		MYL_API key_typed_event(const key_code a_code)
			: key_event(a_code) {}
