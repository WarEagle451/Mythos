...
namespace myl {
	...
	public:
		...
		MYL_API MYL_NO_DISCARD uuid(uuid_version); /// MYTodo: Get to be constexpr
		MYL_API MYL_NO_DISCARD constexpr uuid(u8*);
		MYL_API MYL_NO_DISCARD constexpr uuid(std::string_view);

		MYL_API MYL_NO_DISCARD constexpr bool nil() const;
		...
		MYL_API MYL_NO_DISCARD constexpr bool operator==(const uuid& rhs) const;
		MYL_API MYL_NO_DISCARD constexpr bool operator<(const uuid& rhs) const;
		MYL_API MYL_NO_DISCARD constexpr bool operator>(const uuid& rhs) const;
		MYL_API MYL_NO_DISCARD constexpr bool operator<=(const uuid& rhs) const { return !(*this > rhs); }
		MYL_API MYL_NO_DISCARD constexpr bool operator>=(const uuid& rhs) const { return !(*this < rhs); }
	};
}
