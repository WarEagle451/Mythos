...
namespace mye::vulkan {
    ...
    auto swapchain::destroy(swapchain* h, device& device, VkAllocationCallbacks* allocator) noexcept -> void {
        ...
        // Destroy swapchain (images are also destroyed)

        if (h->m_swapchain)
            vkDestroySwapchainKHR(device.logical(), h->m_swapchain, allocator);
    }

    auto swapchain::recreate(swapchain* h, device& device, VkRenderPass render_pass, const create_info& ci, VkAllocationCallbacks* allocator) -> void {
        vkDeviceWaitIdle(device.logical());

        destroy(h, device, allocator);
        create(h, device, ci, allocator);
        h->recreate_framebuffers(device, render_pass, allocator);
