...
namespace myth::event {
    ...
    public:
        MYL_NO_DISCARD constexpr hid_button_pressed(hid* device, hid_button_code code)
            : m_device{ device }, m_code{ code } {}

        MYL_NO_DISCARD constexpr auto device() const noexcept -> hid* { return m_device; }
    ...
    class hid_button_released : public base {
        ...
        hid_button_code m_code;
    public:
        MYL_NO_DISCARD constexpr hid_button_released(hid* device, hid_button_code code)
            : m_device{ device }, m_code{ code } {}
    ...
    class hid_added : public base {
        const hid::id_type m_id;
        const myl::u16     m_vendor;
        const myl::u16     m_product;
    public:
        MYL_NO_DISCARD constexpr hid_added(hid::id_type id, myl::u16 vendor, myl::u16 product)
            : m_id{ id }
            , m_vendor{ vendor }
            , m_product{ product } {}

        MYL_NO_DISCARD constexpr auto id() const noexcept -> hid::id_type { return m_id; }
    ...
    class hid_removed : public base {
        const hid::id_type m_id;
    public:
        MYL_NO_DISCARD constexpr hid_removed(hid::id_type id)
            : m_id{ id } {}
